...
In this post, let's learn how to protect your ASP NET Core Web API using JWT Bearer Token. We will be using Azure Active Directory (AD) as our Identity Provider and see how to integrate with it from our application and how everything works together.
...
I twill create an API application with a default WeatherForecast Controller that returns some hardcoded data. It also comes with [Swagger Endpoint](https://www.youtube.com/watch?v=3UlCaK9iJaI) setup
...
[.Net Core Web App and Azure AD Security Groups Role-Based Access | Rahul Nath](https://www.rahulpnath.com/blog/dot-net-core-api-and-azure-ad-groups-based-access/)
...
In Microsoft Azure world, Azure Active Directory (AD) is a centralized Identity Provider in the Cloud. It supports and enables various scenarios like Multi-Factor Authentication (sometimes also called 2FA), Single-Sign on, Conditional access (making sure the user is in a specific location, etc.)
...
To use the Implicit flow to get the token from Azure AD, we need to request the OAuth2 authorize endpoint. The URL is available from the Endpoints section under the AD application that we created above. 
...
With this configured, if we rerun the application and call the API, we can see three additional calls to *[login.microsoftonline.com](http://login.microsoftonline.com)* as shown below.
