---
title: 'How To Protect API Gateway REST API With Lambda Authorizers in .NET?'
date: 2022-08-03
tags:
  - AWS
  - Serverless
  - Dotnet-Core
keywords:
description: Lambda Authorizers are a feature provided by API Gateway that helps us separate the authentication logic from our business logic in our function code. Let's explore all about Lambda Authorizers in Amazon API Gateway using .NET Core.
thumbnail: ../images/protect-bird.jpg
popular: true
draft: false
---
...
## Set Up Authorizer in API Gateway
...
![Create a new Lambda Authorizer in API Gateway REST API by providing the name, Lambda function used as the authorizer, and the token type to use. Also, enable token caching if required.](../images/api-gateway-lambda-authorizer-create.jpg)
...
![Set Lambda Authorizer on REST API Method Request](../images/api-gateway-lambda-authorizer-set-authorizer-for-method.jpg)
...
![Sample API gateway REST API request with the token in HTTP Header for Lambda Authorizer. ](../images/api-gateway-lambda-authorizer-sample-api-request-postman.jpg)
...
## Pass Data From Authorizer to Function Code
