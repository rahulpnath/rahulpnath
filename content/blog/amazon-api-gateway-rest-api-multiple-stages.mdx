...
tags:
  ...
  - Dotnet-Core
keywords:
description: Learn how to configure different Stages to use different backend integrations - Lambda Function, DynamoDB etc - based on the Stage they are running. We will use Stage variables to set up connections with different Lambda Functions and DynamoDB database. 
...
I’ve added the same set of permissions that we provided when setting up the first Lambda Function [in the previous post](https://www.rahulpnath.com/blog/amazon-api-gateway-rest-api-lambda-proxy-integration/), but to the stage-specific DynamoDB table.
