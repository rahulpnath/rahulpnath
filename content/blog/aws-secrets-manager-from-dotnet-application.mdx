...
tags:
  ...
  - Serverless
  - Dotnet-Core
description: Learn how to organize and manage Secrets in AWS Secrets manager when building .NET applications. Configure applications such that only secrets are relevant to the applications and retrieved from the AWS Secrets Manager, mainly when it contains secrets for multiple applications.
thumbnail: ../images/aws-secrets-manager-dotnet-application.jpg
popular: true
draft: false
---
...
`youtube:https://www.youtube.com/embed/bBMSL4vInYU`
...
<Bookmark 
    title="Not sure how to connect AWS Secrets Manager with .NET Configuration?" 
...
The `AddSecretsManager` extension method we use in the [previous post](https://www.rahulpnath.com/blog/aws-secrets-manager/), to integrate AWS Secrets Manager with the .NET built-in configuration provider, allows to set a `SecretFilter`. The filter property takes in a function that determines whether or not to retrieve a secret.
