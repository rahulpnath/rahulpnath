...
/> are the stepping stones to building a Serverless application in AWS.
...
As shown below, it contains the same set of files as we saw in the [AWS Lambda](/blog/aws-lambda-net-core/) with the addition of one new file `serverless.template` ;which is the AWS SAM template file.
