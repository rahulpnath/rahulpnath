...
excerpt: >-
  This post describes the different methods by which a client application can
  access Azure Key Vault and use the keys for performing cryptographic
  activities.
tags:
  - Azure
---
> Please check [How the Deprecation of Switch AzureMode Affects Azure Key Vault](/blog/how-the-deprecation-of-switch-azuremode-affects-azure-key-vault/) for scripts using the latest PowerShell cmdlets and check out the new [Azure Key Vault As A Connected Service in Visual Studio 2017](/blog/azure-key-vault-as-a-connected-service-in-visual-studio-2017/).
...
/> [sample](https://github.com/rahulpnath/Blog/tree/master/AzureKeyVault). The code in there uses clientId and secret, you could change it with the above code to use certificate authentication.

> **_<BlogLink
  href="/blog/authenticating-with-azure-key-vault-using-managed-service-identity"
  ...
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/keyvault_managed_service_identity.png"
/> which makes authenticating with it even more easier if your application is deployed in Azure. You no longer have to add any configuration related to key vault to the applications config file._**
