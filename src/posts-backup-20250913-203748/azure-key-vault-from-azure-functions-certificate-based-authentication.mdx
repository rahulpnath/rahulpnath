...
tags:
  - Azure
---
In the previous post we saw how to connect to [Azure Key Vault From Azure Functions](/blog/azure-key-vault-from-azure-functions/). We used the [Authenticating a Client Application with Azure Key Vault](/blog/authenticating-a-client-application-with-azure-key-vault/). Since the general recommendation is to use certificate-based authentication, in this post, we will see how we can use certificates to authenticate from within an Azure Function.
...
Using a certificate to authenticate with the Key Vault is the same as we have [Authenticating a Client Application with Azure Key Vault](/blog/authenticating-a-client-application-with-azure-key-vault/).
...
Make sure you add in the _project.json_ as seen in the [Azure Key Vault From Azure Functions](/blog/azure-key-vault-from-azure-functions/) to enable the required NuGet packages. The Azure function now uses the certificate to authenticate with Key Vault and retrieve the secret.
