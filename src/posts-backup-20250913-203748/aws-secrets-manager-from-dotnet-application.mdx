...
The `AddSecretsManager` extension method we use in the <BlogLink
  href="/blog/aws-secrets-manager"
  title="How Best To Secure Secrets When Building .NET Applications on AWS"
  description="Learn how to get started with using AWS Secrets Manager using a .NET Application. We will learn to connect to Secrets Manager from .NET using the client SDK and retrieve secrets. We will also see how to integrate Secrets Manager into built-in .NET Configuration and seamlessly use secrets from our ap"
  authorImage="/images/logo-512x512-24.png"
  ...
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/secure-padlock.jpg"
/>, to integrate AWS Secrets Manager with the .NET built-in configuration provider, allows to set a `SecretFilter`. The filter property takes in a function that determines whether or not to retrieve a secret.
