...
date: '2018-06-26'
feature_image: /images/postman_collection_runner.png
excerpt: Quick and easy way to test your API.
tags:
  - Testing
---
A while back we looked at how we can use [Postman](/blog/postman-chaining-requests-to-speed-up-manual-api-tests/). For those who are not familiar with [Postman](https://www.getpostman.com/), it is an application that assists in API testing and development, which I see as sitting a level top of a tool like [Fiddler](/blog/fiddler-free-web-debugging-proxy/).
...
Postman allows executing scripts before and after running API requests. We did see this in the [Postman](/blog/postman-chaining-requests-to-speed-up-manual-api-tests/) where we grabbed the _messageId_ from the POST request and added it to the environment variable for use in the subsequent requests. Similarly, we can also add scripts to verify that the API request returns expected results, status code, etc.
...
```
![Postman Tests](/images/postman_tests.png)
...
![Postman Collection Runner](/images/postman_collection_runner.png)
