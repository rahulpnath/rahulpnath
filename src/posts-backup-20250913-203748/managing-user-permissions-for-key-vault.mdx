...
Azure Key Vault gets created in the default AD associated with the subscription, so we need to add the new user to that. (If you are not sure on how to find the default AD [Authenticating a Client Application with Azure Key Vault](/blog/authenticating-a-client-application-with-azure-key-vault/) describes it in the beginning). In the portal under the _Azure Directory_ option, select the default directory and on the _Users_ tab, we can add a new user.
...
To create a key vault that we want to give permissions for the user, the below powershell scripts can be used. If you are new to key vault, then check out the [Getting Started with Azure Key Vault](/blog/getting-started-with-azure-key-vault/) or [other related articles](/tag/azure-key-vault/).
