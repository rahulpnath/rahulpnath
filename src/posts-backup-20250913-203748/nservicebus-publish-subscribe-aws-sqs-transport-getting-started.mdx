...
date: '2023-04-18'
feature_image: /images/NServiceBus-Events-on-AWS-SQS--1-.png
excerpt: >-
...
In an earlier blog post, we learned [previous post](/blog/getting-started-with-nservicebus-on-aws/). We learned how to Install and set up NServiceBus, set up the receiver and sender to listen to messages on Amazon SQS Transport, and communicate between the two using Commands.

In this blog post, let's explore how to publish Events using NServiceBus.

<iframe src="https://www.youtube.com/embed/6yW9Spw9j_c?feature=oembed" />
...
Following up with the same <BlogLink
  href="/blog/getting-started-with-nservicebus-on-aws"
  title="NServiceBus on AWS SQS: Learn How to Quickly Get Started"
  description="NServiceBus is a messaging framework provided by Particular Software, which makes it quick and easy to build message-driven applications. Learn how to quickly get started by building a sender/receiver using NServiceBus on AWS."
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/NServiceBus-on-AWS-SQS.png"
/>, we can raise a `UserCreatedEvent` inside our `CreateUserCommandHandler` every time a user is successfully created.
...
The Startup code for `payroll-service` is very similar to how we [previous post](/blog/getting-started-with-nservicebus-on-aws/)
...
NServiceBus uses a combination of Amazon SNS and SQS to deliver Events to multiple subscribers. An SNS Topic is automatically created for the Event, and it adds a subscription to the SQS Queue that the service listens on.
...
Multiple Subscriptions are created on the SNS Topic to deliver a copy of the same Event message to different Subscribers.
...
In the previous post, we saw how to configure this when dealing with [previous post](/blog/getting-started-with-nservicebus-on-aws/)
