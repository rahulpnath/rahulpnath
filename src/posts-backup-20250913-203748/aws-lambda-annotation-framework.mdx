...
There are [AWS Lambda](/blog/aws-lambda-dotnet-developer/) on AWS Lambda, including using API Gateway, Function URLs, and hosting ASP.NET Core Applications.
...
-   `RestApi` → Configures Lambda Function to be called from [previous post](/blog/amazon-api-gateway-rest-api-introduction/).
-   `HttpApi` → Configures Lambda Function to be called from [HTTP APIs](/blog/amazon-api-gateway-http-apis/).
