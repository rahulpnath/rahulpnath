...
I can still have the Customer instance injected via the constructor and override just the FirstName and LastName properties, as shown above. AutoFixture [Populating Data for Tests](/blog/populating-data-for-tests/) to configure how objects get created. You can take advantage of it with SpecFlow, by overriding the fixture used in the `DomainDataAutoPopulatedSource` class.
