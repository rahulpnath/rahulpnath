...
In an earlier blog post, we learned [Amazon SNS and AWS Lambda Triggers in .NET](/blog/amazon-sns-lambda-trigger-dotnet/). I highly recommend checking out if you are new to SNS and Lambda Triggers.
...
I cover these configuration details in detail in my post on [Amazon SNS and AWS Lambda Triggers in .NET](/blog/amazon-sns-lambda-trigger-dotnet/).
...
-   On errors in processing, the message is replaced into the Queue. <BlogLink
...
/> back into the original queue for reprocessing.
-   SQS allows Batch delivery of messages, so you can save on the cost of the number of Lambda functions to run. Along with batch delivery, you can also [How to Handle Exceptions When Processing SQS Messages in .NET Lambda Function](/blog/amazon-sqs-lambda-trigger-exception-handling-dotnet/) so that only messages that error out in a batch is retried.
-   You can control the Lambda concurrency, and you run fewer Lambda instances with batching.
-   SQS supports `BatchWindow`, so you can [Amazon SQS and AWS Lambda Triggers in .NET](/blog/amazon-sqs-lambda-trigger-dotnet/) before starting to processing.
