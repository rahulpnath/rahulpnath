---
title: 'Review: Pixel, Phone by Google - Made by Google'
slug: review-pixel
date: '2017-07-12'
feature_image: /images/pixel.png
excerpt: Worth considering if you are looking out for a new phone.
tags:
  - Productivity
---
When it comes to mobile phones, I used to be a gadget freak and buy the latest phone as soon as it arrives in the market. But not after I got the [previous review](/blog/review-two-months-and-counting-android-and-nexus-5/). It's been well over three years now and not even once did I want to switch over to something else. The phone never slowed down over these years, and the battery lasted enough for the day. I broke the Nexus screen a couple of months back and was forced to switch over to a new phone. I didn't have to do much market research or phone comparisons this time; as I just knew which one to pick up.

[The Pixel, Phone By Google!](https://madeby.google.com/intl/en_au/phone/)

![Pixel](/images/pixel.png)

## Key Features

The Pixel is the first phone in the Google Pixel hardware line and is a successor to the Nexus range of devices. The Pixel runs [Android-Nougat](https://www.android.com/intl/en_au/versions/nougat-7-0/) with some features exclusive to it. The Pixel is very much comparable to the Nexus, with improved hardware and latest software. Below are some of the key features that I like about the phone.

-   **Fingerprint sensor** The fingerprint sensor on the back allows you to unlock the phone and specific apps and also authorize purchases. <BlogLink
  href="/blog/password-manager-get-one-if-you-havent-already"
  title="Tip of the Week: Password Manager - Get One If you Haven't Already"
  description="The only secure password is the one you can’t remember"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/password_manager.jpg"
/> is integrated with the sensor which enables me to unlock it using the sensor. I no longer need to type in my long master password every time I use 1Password.
-   **Camera** _Pixel takes brilliant photos in bright light, low light, and any light._ The camera is awesome and at times is a good replacement for your [Photography - Learning to Click](/blog/photography-learning-to-click/). With Pixel, Google also provides unlimited original quality storage for all photos and videos.
-   **Google Assistant** Pixel is the first phone with the [Google Assistant](https://assistant.google.com/) built-in. It helps you manage tasks, find content on phone and the internet, perform actions, etc. just like Siri or Cortana. I have not used the assistant much and yet to find some valid use cases.

#### **Apps**

Some of the apps that I use regularly are

-   <BlogLink
  href="/blog/todoist-manage-your-todo-list"
  title="Tip of the Week: Todoist: Manage your To-Do List"
  description="A To-Do list manager that has been working for me over two years."
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/todoist_logo.jpg"
/>: My ToDo List
-   <BlogLink
  href="/blog/feedly-one-stop-reading-place"
  title="Tip of the Week: Feedly - One Stop Reading Place"
  description="Read this interesting blog post"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/feedly_site.png"
/>: For all my RSS feeds
-   <BlogLink
  href="/blog/pocket-offline-reading-made-easy"
  title="Tip of the Week: Pocket - Don't Miss Out on the Articles That You Want to Read Later"
  description="Manage articles that you want to read, but just don't have the time for it right now!"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/pocket.jpg"
/>: For offline reading and read later
-   <BlogLink
  href="/blog/password-manager-get-one-if-you-havent-already"
  title="Tip of the Week: Password Manager - Get One If you Haven't Already"
  description="The only secure password is the one you can’t remember"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/password_manager.jpg"
/>: Password Manager
-   <BlogLink
  href="/blog/buffer-smarter-social-sharing"
  title="Tip of the Week: Buffer - Smarter Social Sharing"
  description="Read this interesting blog post"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/buffer.png"
/>: To schedule articles to all social networks
-   <BlogLink
  href="/blog/rescue-time-track-your-time"
  title="Tip of the Week: Rescue Time - Track Your Time"
  description="Wondering where your time goes? Start Tracking!"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/rescuetime.png"
/>: Automatically tracks where I spent time on mobile
-   <BlogLink
  href="/blog/three-months-to-a-half-marathon"
  title="How I Went From Couch to Half Marathon in Twelve Weeks"
  description="Update on how I have been going about my running."
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/running_half_marathon.jpg"
/>: Tracking my runs and exercises
-   [Kindle](https://play.google.com/store/apps/details?id=com.amazon.kindle): Reading kindle books
-   Twitter, Whatsapp, Messenger, Instagram, Skype & Yammer :Social Networking apps
-   <BlogLink
  href="/blog/optimizing-octopress-workflow-for-new-posts"
  title="Optimizing Octopress Workflow for New Posts"
  description="Read this interesting blog post"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/default-feature.jpg"
/>: Write articles for blog in markdown format
-   <BlogLink
  href="/blog/how-to-setup-google-indic-keyboard-on-your-android-phone"
  title="Type in Your Native Indian Language - Setting up Indic Keyboard on Your Android Phone"
  description="Write in your native language!"
  authorImage="/images/logo-512x512-24.png"
  authorName="Rahul Nath"
  authorTitle="Rahul Pulikkot Nath"
  featureImage="/images/indickeyboard.png"
/>: Writing in my native language
-   Gmail & Outlook: Emails
-   Youtube: Videos

#### \*\*Case \*\*

I am using the [Tip of the Week: Spigen - Awesome Phone Cases](/blog/spigen-awesome-phone-cases/) case for Pixel. It adds extra weight to the phone, but I am used to it from my Nexus as well. The Neo Hybrid provides military grade protection and protects the phone from most falls.

Overall I have found the Pixel a good phone and recommend it to anyone looking out for a new phone now. The Samsung Galaxy S8 is also of the same range (slightly costlier) and worth considering if you are not too particular about having a vanilla Android experience. Which phone do you use?
