...
The [Lambda Annotations Framework](/posts/aws-lambda-annotation-framework/) is a programming model that makes it easier to build AWS Lambda Functions using .NET.
...
In a [previous post](/posts/aws-lambda-annotation-framework/), we learned how to get started with using the Annotations Framework and how it compares to using the default Lambda Programming model. The Annotations Framework removes boilerplate code required to integrate with API Gateway.
