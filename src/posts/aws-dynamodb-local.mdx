...
tags: ["AWS","DynamoDB"]excerpt: "Learn how to set up and run DynamoDB database on your local development machine. We'll see how to connect using AWS CLI, from .NET application and using GUI tools like Dynobase and NoSQL Workbench."
...
If you are on the Windows platform, you can use [Chocolatey](/posts/chocolatey-install-softwares-with-ease/), which is a Package Manager for Windows. It makes installing software a breeze, including [awscli](https://community.chocolatey.org/packages/awscli) → `choco install awscli`
...
In an earlier post, I had walked through in detail how to get started with [DynamoDB from .NET.](/posts/aws-dynamodb-net-core/)
