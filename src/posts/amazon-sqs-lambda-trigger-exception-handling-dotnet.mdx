...
In a previous post, we learned how to [process messages coming to Amazon SQS using Lambda Functions](/posts/amazon-sqs-lambda-trigger-dotnet/) written in .NET.
...
The message is not deleted from the Queue by the Lambda Function. After the [default VisbilityTimeout](/posts/amazon-sqs) period of the Queue (30secs), the message is made available in the Queue for processing again.
