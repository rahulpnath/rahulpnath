...
feature_image: "/images/aws-lambda-dynamodb-streams-trigger-exception-handling-dotnet.jpg"
...
In a previous blog post, [DynamoDB Streams and AWS Lambda Trigger in .NET](/posts/aws-dynamodb-streams-lambda-triggers-dotnet/), we learned how to set up DynamoDB Stream on a DynamoDB table and wire up a Lambda Function written in .NET to respond to stream record events.
...
Below I have modified the Lambda Function [from our previous blog post](/posts/aws-dynamodb-streams-lambda-triggers-dotnet/), which listens to changes in my User table, to check if the user’s Name property contains the word ‘Error’. And if it does, I throw an exception.
