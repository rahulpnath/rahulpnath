...
Add this to your PowerShell profile file (run notepad $PROFILE) as we did for theming when we [set up the windows terminal](__GHOST_URL__/blog/setting_up_windows_terminal/). In the above script, I write to the '_$aliasFIlePath_' and load all the alias from that file using the Dot sourcing operator.
