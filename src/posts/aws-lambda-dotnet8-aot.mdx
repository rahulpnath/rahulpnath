...
The [AWS Toolkit](/posts/aws-lambda-net-core/#setting-up-first-project) has been updated to create .NET 8 Lambda functions, which makes it easy to create new projects.
...
You can also use the [Mock Lambda test tool](/posts/aws-lambda-net-core/#mock-lambda-test-tool) to run and test your functions locally.
...
You can find the Lambda Init duration in [CloudWatch logs](/posts/amazon-cloudwatch-logs-dotnet/), every time a new instance of the function is created. If that entry is missing, it means you are reusing a Lambda instance.
