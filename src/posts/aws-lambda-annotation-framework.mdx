...
<iframe src="https://www.youtube.com/embed/qWxjCkqX\_lM?feature=oembed" />
...
There are [several ways to build APIs](/posts/aws-lambda-dotnet-developer/#building-apis-with-aws-lambda) on AWS Lambda, including using API Gateway, Function URLs, and hosting ASP.NET Core Applications.
...
*   `RestApi` → Configures Lambda Function to be called from [API Gateway REST API](/posts/amazon-api-gateway-rest-api-introduction/).
*   `HttpApi` → Configures Lambda Function to be called from [API Gateway HTTP API](/posts/amazon-api-gateway-http-apis/).
