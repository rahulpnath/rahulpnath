...
date: '2022-09-12'
feature_image: /content/images/amazon-s3-lambda-triggers-dotnet.jpg
tags:
  - AWS
  - Lambda
excerpt: >-
  Amazon S3 raises event notifications when Objects are created and modified.
  Learn how to use this to trigger Lambda Functions in .NET and the different
  configuration associated with processing the notification messages.
---
...
<ImageCard src="/content/images/s3-add-lambda-trigger.jpg" alt="Add Trigger from the Lambda Function under Configuration → Triggers. This allows to choose different sources for the Lambda Trigger, and one of them is S3." />
...
<ImageCard src="/content/images/s3-add-lambda-trigger-create.jpg" alt="Select S3 and Lambda Trigger source and select the S3 Bucket that serves as the event source. Specify the event types for which to trigger the .NET Lambda Function" />
...
<ImageCard src="/content/images/s3-trigger-lambda-invoke-permission.jpg" alt="The resource-based policy allows S3 to invoke the .NET Lambda Function on a notification." />
...
<ImageCard src="/content/images/s3-lambda-iam-permissions.jpg" alt="Update the Lambda IAM Permissions to talk to the S3 Bucket. To read the file data and the metadata, assign the GetObject Read access." />
...
<ImageCard src="/content/images/s3-add-lambda-trigger-configuration.jpg" alt="S3 Lambda Trigger configuration to specify the Prefix or Suffix, to filter the files sent to Lambda Function." />
...
<ImageCard src="/content/images/s3-lambda-asynchronous-invocation-configuration.jpg" alt="Lambda Asynchronous configuration sets the message retry limit and the maximum age event." />
...
<ImageCard src="/content/images/s3-lambda-asynchronous-invocation-configuration-edit.jpg" alt="Update the Lambda Asynchronous Configuration to set up the Dead Letter Queue. Any messages that cannot be processed after the specified number of retries is moved to the DLQ" />
