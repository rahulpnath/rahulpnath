...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-proxy-stage-variables.jpg" alt="Add stage variable to the API Stage. " />
...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-proxy-lambda-method-integration-request-stage-variables.jpg" alt="Use stage variables to prefix the Lambda Function name when specifying the Integration endpoint in API Gateway REST API Integration Method." />
...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-proxy-lambda-permissions.jpg" alt="Add permission to Lambda Function manually for API Gateway to be able to invoke it. When you use stage variables in the lambda name, this dialog prompts up since you need to add in the permissions manually." />
...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-proxy-lambda-environment-variable.jpg" alt="Add environment variable for the Lambda Function from the AWS Console." />
...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-proxy-lambda-iam-role-stage.jpg" alt="Update the Lambda IAM Role to provider permissions to talk to the Stage-specific DynamoDB table. Explicitly select the required permissions and the Resources the Lambda function needs access to." />
...
<ImageCard src="/content/images/api-gateway-rest-api-lambda-stage-permission.jpg" alt="Add a resource-based policy to provide the API Gateway permission to invoke the Lambda Function. The Source ARN specifies the endpoint ARN details that require access. In this case, all HTTP Methods under the user resource is given access to invoke the Lambda function. " />
...
<ImageCard src="/content/images/api-gateway-rest-api-deployment-history.jpg" alt="Curernt and previous deployment history under API Gateway Stages. Select a different version and deploy that version." />
