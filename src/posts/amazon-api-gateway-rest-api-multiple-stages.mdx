...
In a previous post, we learned [how to build an API Gateway REST API Using AWS Lambda Proxy Integration](/posts/amazon-api-gateway-rest-api-lambda-proxy-integration/). We learned how to handle the API Gateway requests in a Lambda function built in .NET, connect to a DynamoDB table and perform CRUD operations.
...
![Add environment variable for the Lambda Function from the AWS Console.](/images/api-gateway-rest-api-lambda-proxy-lambda-environment-variable.jpg)
...
I’ve added the same set of permissions that we provided when setting up the first Lambda Function [in the previous post](/posts/amazon-api-gateway-rest-api-lambda-proxy-integration/), but to the stage-specific DynamoDB table.
...
![Add a resource-based policy to provide the API Gateway permission to invoke the Lambda Function. The Source ARN specifies the endpoint ARN details that require access. In this case, all HTTP Methods under the user resource is given access to invoke the Lambda function. ](/images/api-gateway-rest-api-lambda-stage-permission.jpg)
