...
To create a key vault that we want to give permissions for the user, the below powershell scripts can be used. If you are new to key vault, then check out the [Getting Started with Azure Key Vault](/posts/getting-started-with-azure-key-vault/) or [other related articles](/blog/tag/azure-key-vault/).
