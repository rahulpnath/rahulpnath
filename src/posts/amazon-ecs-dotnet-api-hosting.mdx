...
date: "2024-07-29T06:41:31.000Z"
feature_image: "/images/ECS-ASP-NET.png"
tags: ["AWS"]
excerpt: "Let's learn some of the core concepts of ECS, while deploying an ASP NET Web API application on Amazon ECS using Fargate.  we will explore some key components of ECS, including task definitions, tasks, services clusters, etc."
...
Thank you AWS for sponsoring this post in the [ECS Series](/posts/tag/ecs/).
...
Core Concepts of ECS and how they are related
...
Publish to AWS option to deploy ASP NET API to ECS Fargate from within Visual Studio
...
Visual Studio AWS Toolkit specify health check endpoint for ECS Fargate cluster
...
Visual Studio AWS Toolkit status of deploying .NET API to ECS with Fargate
...
Status of the CloudFormation template being run as part of the CDK deploy step from AWS Toolkit
...
ECS Cluster details with 1 service running.
...
Details of the Service running in our ECS cluster, which has the load balancer attached to it.

You can also see that the Tasks are running on Fargate instances.

![](/images/image-23.png)

Tasks running as part of the ECS service, which all points to the same Task Definition file.
...
Task Definition showing the memory, CPU, the docker image used etc.
...
Clean up all resources created by deleting the entire CloudFormation stack.