...
In an earlier blog post, we learned [how to quickly start with NServiceBus on AWS SQS](/posts/getting-started-with-nservicebus-on-aws/). We learned how to Install and set up NServiceBus, set up the receiver and sender to listen to messages on Amazon SQS Transport, and communicate between the two using Commands.

In this blog post, let's explore how to publish Events using NServiceBus.

<iframe src="https://www.youtube.com/embed/6yW9Spw9j\_c?feature=oembed" />
...
Following up with the same [code example from our getting started blog](/posts/getting-started-with-nservicebus-on-aws/), we can raise a `UserCreatedEvent` inside our `CreateUserCommandHandler` every time a user is successfully created.
...
The Startup code for `payroll-service` is very similar to how we [set up the Receiver endpoints on Amazon SQS Transport.](/posts/getting-started-with-nservicebus-on-aws/#setting-up-receiver-on-amazon-sqs-transport)
...
![](/images/amazon-sns-to-lambda-or-sns-sqs-lambda-dotnet.jpg)

](/posts/amazon-sns-to-lambda-or-sns-sqs-lambda-dotnet/)
...
In the previous post, we saw how to configure this when dealing with [large Message Sizes with NServiceBus and SQS Transport.](/posts/getting-started-with-nservicebus-on-aws/#large-message-sizes-with-nservicebus-and-sqs-transport)
