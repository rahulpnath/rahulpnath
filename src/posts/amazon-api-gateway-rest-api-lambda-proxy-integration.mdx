...
Since I have set up the AWS Credentials for my local development environment using configuration files, I don’t have to pass in any credentials explicitly. If you want to set it up for your development environment, check out my blog post on [How To Manage Credentials When Building .NET Application on AWS](/posts/amazon-credentials-dotnet/).
...
Once you create a new API Gateway REST API, add a User resource and a `GET` method under that. You can follow the steps detailed [in my previous post](/posts/amazon-api-gateway-rest-api-introduction/) to get this setup.
