...
![](/images/Lambda-Annotation-Framework---CRUD.png)

](/posts/lambda-annotation-framework-crud-api/)

All we need to do is add the [Lambda Annotation Framework](/posts/aws-lambda-annotation-framework/) NuGet package and wrap our Pass Generation function with the `LambdaFunction` attribute. Since this is going to be exposed as an API will also add the `HttpApi` attribute and specify the route and the method `Get`.
...
The `HttpApi` attribute allows us to expose the Lambda Function over a [Lambda Function URL](/posts/function-urls-in-aws-lambda-dotnet/) or an [HTTP API Gateway endpoint](/posts/amazon-api-gateway-http-apis/).
