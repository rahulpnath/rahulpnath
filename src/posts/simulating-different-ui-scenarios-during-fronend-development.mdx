...
In a previous post, [Simulating Different Scenarios Using Fake JSON Server API](/posts/simulating-different-api-scenarios-using-json-server/), I showed how to set up a fake API to return data based on different UI state. E.g., Given a UI list view, the application can be in different states. It can show an empty list, a list of data, a list of data that does not fit in one page, a server error, etc.
