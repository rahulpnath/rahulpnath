...
date: '2024-03-17'
feature_image: /content/images/2024/03/RabbitMQ---Direct-Exchange.png
tags:
  - RabbitMQ
  - AWS
excerpt: >-
  A Direct Exchange routes messages to queues whose binding key matches the
  message routing key. Let's understand how to set up a Direct Exchange and how
  it routes messages to consumers.
---
...
<ImageCard src="/content/images/2024/03/image-1.png" alt="Shows two queues bound to a Direct Exchange, with different binding keys (represented by the green and orange color lines)" caption="Shows two queues bound to a Direct Exchange, with different binding keys (represented by the green and orange color lines)" />
...
<ImageCard src="/content/images/2024/03/image-2.png" alt="The same queue can have multiple routing keys, and the same routing key can be bound to multiple Queues. The green routing key is sent to two Queues, and the Queue on the right gets messages with 'green' and 'orange' routing keys." caption="The same queue can have multiple routing keys, and the same routing key can be bound to multiple Queues. The green routing key is sent to two Queues, and the Queue on the right gets messages with 'green' and 'orange' routing keys." />
