...
In the previous post we saw how to connect to [Azure Key Vault from Azure Functions](/posts/azure-key-vault-from-azure-functions/). We used the [Application Id and Secret to authenticate with the Azure AD Application](/posts/authenticating-a-client-application-with-azure-key-vault/). Since the general recommendation is to use certificate-based authentication, in this post, we will see how we can use certificates to authenticate from within an Azure Function.
...
Using a certificate to authenticate with the Key Vault is the same as we have [seen before](/posts/authenticating-a-client-application-with-azure-key-vault/).
...
Make sure you add in the _project.json_ as seen in the [previous post](/posts/azure-key-vault-from-azure-functions/) to enable the required NuGet packages. The Azure function now uses the certificate to authenticate with Key Vault and retrieve the secret.
