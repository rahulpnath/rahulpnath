...
date: '2022-08-17'
feature_image: /content/images/aws-lambda-dynamodb-streams-trigger-dotnet.jpg
tags:
  - AWS
  - DynamoDB
  - Lambda
excerpt: >-
  DynamoDB Streams capture a time-ordered sequence of events in any DynamoDB
  table. Let's learn how to enable DynamoDB Streams, different stream types and
  how to consumer stream changes from a .NET AWS Lambda Function.
---
...
<ImageCard src="/content/images/dynamodb-stream-enable-existing-table.jpg" alt="Enable DynamoDB Stream on a Table under the Exports and streams section in the AWS Console." />
...
<ImageCard src="/content/images/dynamodb-stream-enable-options.jpg" alt="Select View Type when enabling DynamoDB Stream. This decides the data that is available as part of the stream events raised on changed to items in DynamoDB Table. " />
...
<ImageCard src="/content/images/dynamodb-stream-details.jpg" alt="Once enabled, you can see the Stream details, view type, and attached Triggers under the DynamoDB table details. " />
...
[AWS Lambda](/blog/aws-lambda-net-core/) is Amazon’s answer to serverless computing services. It allows running code with zero administration of the infrastructure that the code is running on.

Make sure to install the AWS Toolkit ([Visual Studio](https://aws.amazon.com/visualstudio/), [Rider](https://aws.amazon.com/rider/), [VS Code](https://aws.amazon.com/visualstudiocode/)) and [set up Credentials for the local development environment](/blog/amazon-credentials-dotnet/).  This helps you quickly develop a .NET application on the AWS infrastructure and deploy them quickly to your AWS Accounts.
...
To publish the Lambda function to the AWS account, you can use the Publish to AWS Lambda option that the AWS Toolkit provides. You can directly publish the new lambda function with your [development environment connected to your AWS account](/blog/amazon-credentials-dotnet/).
...
<EmbedCard src="https://www.youtube.com/embed/U96230YlSJA" />
...
<ImageCard src="/content/images/dynamodb-stream-add-lambda-trigger.jpg" alt="Enable Lambda Trigger for a .NET Function on DynamoDB Stream." />
...
<ImageCard src="/content/images/dynamodb-stream-lambda-triggers.jpg" alt="DynamoDB Trigger created under the .NET Lambda function can be seen from the Lambda function in the AWS Console." />
...
<ImageCard src="/content/images/dynamodb-stream-lambda-trigger-log.jpg" alt="Logs in Amazon CloudWatch after updating an item in the DynamoDB table. It triggers the .NET Lambda function, which writes the details to the log." /><BookmarkCard href="/blog/amazon-cloudwatch-logs-dotnet" title="How To Log Correctly To AWS CloudWatch From a .NET Application" description="Learn how to use Amazon CloudWatch when building .NET applications. We will learn how to write logs into CloudWatch, filter the logs, some good practices when logging, learn about Log Analytics, and see how we can filter and view logs across multiple applications." icon="/favicon.ico" />
...
<ImageCard src="/content/images/dynamodb-stream-lambda-trigger-edit-configuration.jpg" alt="Setting up Lambda Trigger configuration for the DynamoDB Trigger, batch size and batch window, on the .NET Function." />
...
<ImageCard src="/content/images/dynamodb-stream-lambda-trigger-filter.jpg" alt="JSON Filtering Rule on the DynamoDB Lambda Trigger helps control what records are passed on to the .NET function." />
...
<BookmarkCard href="/blog/aws-dynamodb-streams-lambda-triggers-exception-handling-dotnet" title="How to Handle Exceptions When Processing DynamoDB Stream Events in .NET Lambda Function" description="This article is sponsored by AWS and is part of my AWS Series. DynamoDB Streams capture a time-ordered sequence of events in any DynamoDB table. In a previous blog post, DynamoDB Streams and AWS Lambda Trigger in .NET, we learned how to set up DynamoDB Stream on a DynamoDB table" icon="/favicon.ico" />
