...
The `AddSecretsManager` extension method we use in the [previous post](/posts/aws-secrets-manager/), to integrate AWS Secrets Manager with the .NET built-in configuration provider, allows to set a `SecretFilter`. The filter property takes in a function that determines whether or not to retrieve a secret.
