...
The below code no longer uses a Connection String to connect to the Azure Queue. When [running on the local machine](/posts/azure-managed-service-identity-and-local-development/), it can automatically pick up the user logged in to Visual Studio or other supported IDE's. If the application is running in Azure infrastructure, enable Identity on the resource and grant it permissions using the Access Control (IAM).
