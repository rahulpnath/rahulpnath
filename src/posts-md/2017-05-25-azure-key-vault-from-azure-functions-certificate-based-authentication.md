...
date_updated: 2024-11-28T02:19:16.000Z
tags: Azure
excerpt: Authenticate with Azure Key Vault from Azure Function using Certificates
---

In the previous post we saw how to connect to [Azure Key Vault from Azure Functions](__GHOST_URL__/blog/azure-key-vault-from-azure-functions/). We used the [Application Id and Secret to authenticate with the Azure AD Application](__GHOST_URL__/blog/authenticating-a-client-application-with-azure-key-vault/). Since the general recommendation is to use certificate-based authentication, in this post, we will see how we can use certificates to authenticate from within an Azure Function.
...
![Azure Function New App](__GHOST_URL__/content/images/azureFunction_newFunctionApp.png)
...
![Azure Function Platform Features](__GHOST_URL__/content/images/azureFunction_PlatformFeatures.png)
Upload the certificate by selecting it from your folder system.
![Azure Function Upload Certificate](__GHOST_URL__/content/images/azureFunction_addCertificate.png)
...
![Azure Function Certificates App Settings](__GHOST_URL__/content/images/azureFunction_AppSetting.png)
Using a certificate to authenticate with the Key Vault is the same as we have [seen before](__GHOST_URL__/blog/authenticating-a-client-application-with-azure-key-vault/).
...
Make sure you add in the *project.json* as seen in the [previous post](__GHOST_URL__/blog/azure-key-vault-from-azure-functions/) to enable the required NuGet packages. The Azure function now uses the certificate to authenticate with Key Vault and retrieve the secret.
